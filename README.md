...
2. *Containerize*: In project directory, run "gcloud auth login" and log into Google Cloud Platform (GCP) using web browswer
3. *Upload image to Container Registry*: In project directory, run "gcloud builds submit --tag gcr.io/<PROJECT-ID>/<REGISTRY NAME>", where <PROJECT-ID> is ID generated by GCP for your project (usually project_name-number_sequence), <REGISTRY NAME> is name of your choice (usually project name) ... confirm that image successfully created with STATUS "SUCCESS"
